...
directory = r'c:\Users\HP\OneDrive\Desktop\ncc\publish\FAQs'

# Set the working directory
desired_dir = r'c:\Users\HP\OneDrive\Desktop\ncc\ncc_chatbot\my-app\packages\pirate-speak\pirate_speak'
...
def embedding_document(embedding_model):
    ...
    # Create and persist vectorstore using Pincone
    try:
        vectorstore = PineconeVectorStore.from_documents(index_name=index_name,
            documents=texts_split, embedding=embedding_model)
        end_time = time.time()
        print(f"Time Taken is: {end_time - start_time}")
        print("Done creating Vector database")
        return vectorstore  # Return the created vectorstore
...
vectorstore = embedding_document(embedding)
